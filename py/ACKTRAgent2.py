...
class SunhoAgent:
    ...
    def define_graph(self):
        ...
        fc_layer = self.fully_connected_layer(self.x_batch,self.state_size, 100, 'fc_layer1')
        fc_layer = tf.nn.relu(fc_layer)
        fc_layer = self.fully_connected_layer(fc_layer,100, 50, 'fc_layer2')
        fc_layer = tf.nn.relu(fc_layer)
        fc_layer = self.fully_connected_layer(fc_layer,50, 30, 'fc_layer3')
        fc_layer = tf.nn.relu(fc_layer)
        #policy output layer
        self.policy_logits = self.fully_connected_layer(fc_layer, 30, self.num_actions, 'policy_logits')
        self.policy_probs = tf.nn.softmax(self.policy_logits)
        
        #value output layer
        self.value_preds = self.fully_connected_layer(fc_layer, 30, 1, 'value_fc_layer')
